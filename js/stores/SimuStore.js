...
class SimuStore extends ReduceStore {
    ...
    generateSeries(salaires, retraite, chomage, couple, nbEnfants) {
        ...
        const netSum = salaires.person1 + salaires.person2;

        const userParams = UserParams(netSum, retraite, chomage, couple, nbEnfants)
        const irParams = IRParams(userParams)

        const simulation = Simulation(salaires.person1, salaires.person2, nbEnfants, couple)
        const jlmSimulation = JLMSimulation(salaires, couple, nbEnfants, simulation.salaireBrut);
        
        var IR = Math.round(simulation.impot.du.value  * 12),
              CSG = Math.round(simulation.csg.du.value  * 12),
              NEW_IR = Math.round(jlmSimulation.ir * 12),
              CSG_P = Math.round(jlmSimulation.csg * 12);

        var gain = (IR + CSG) - (NEW_IR + CSG_P)
        if (gain < 0) {
            if (couple === 0) {
                if (nbEnfants > 0 && salaires.person1 >= 2000 && salaires.person1 <= 5000) {
                    var newSimulation = Simulation(salaires.person1, salaires.person2, 0, 0)
                    var newSalaires = { "person1": salaires.person1}
                    var newJLMSimulation = JLMSimulation(newSalaires, 0, 0, newSimulation.salaireBrut)
                    var new_gain = 12 * (newSimulation.impot.du.value  + newSimulation.csg.du.value) - 12 * (newJLMSimulation.ir + newJLMSimulation.csg)
                    NEW_IR  = - Math.round((new_gain - IR - CSG + CSG_P))

                }
            } else {
                var sommeSalaires = salaires.person1 + salaires.person2
                var salaireMax = Math.max(salaires.person1, salaires.person2)
                var salaireMin = Math.min(salaires.person1, salaires.person2)
                var ecart = salaireMax - salaireMin
                var newSalaire = { "person1": (salaireMin + salaireMax)/2 }

                if (nbEnfants == 0 & salaireMax >= 1400) {
                    if (salaireMax <= 4000) {
                        var newSimulation = Simulation(newSalaire.person1, newSalaire.person2, 0, 0)
                        var newJLMSimulation = JLMSimulation(newSalaire, 0, 0, newSimulation.salaireBrut)
                        var new_gain = 12 * (newSimulation.impot.du.value  + newSimulation.csg.du.value) - 12 * (newJLMSimulation.ir + newJLMSimulation.csg)
                        //if (new_gain > gain) {
